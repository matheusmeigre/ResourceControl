  ...
  return (
    <>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="grid grid-cols-[2fr_1fr_2fr_1fr_auto] px-5 py-3 bg-gray-50 border-b border-gray-100 gap-4">
          ...
          {canEdit && <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Ações</span>}
        </div>
        <div className="divide-y divide-gray-50">
          ...
          {filtered.map((app) => (
            <div key={app.id} className="grid grid-cols-[2fr_1fr_2fr_1fr_auto] px-5 py-3 gap-4 items-center hover:bg-gray-50">
              <div className="flex items-center gap-2 min-w-0">
                <span className="font-mono text-sm font-semibold text-gray-800 truncate">{app.name}</span>
                {app.repoUrl && (
              ...
              <span className="font-mono text-xs text-gray-500 truncate">{app.defaultBranch}</span>
              <span className="text-sm text-gray-500 truncate">{app.notes ?? "—"}</span>
              <span>
                <span
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm mx-4">
            <h2 className="text-lg font-semibold text-gray-900 mb-2">Excluir aplicação?</h2>
            <p className="text-sm text-gray-500 mb-5">
              Tem certeza que deseja excluir{" "}
              <strong className="text-gray-800">{confirmDelete.name}</strong>? Esta ação não
              poderá ser desfeita.
            </p>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Branch padrão</label>
            <input
              type="text"
              value={defaultBranch}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">URL do repositório</label>
            <input
              type="url"
              value={repoUrl}
